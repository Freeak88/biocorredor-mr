...
test.describe('D.1 demo de campo contra backend real', () => {
  ...
  test('observador guarda, reabre offline y sincroniza', async ({ page, context }, testInfo) => {
    ...
    await expect(page.getByText('Observadora Demo 1')).toBeVisible();
    ...
    await expect(page.getByText('SEC-CENTRO', { exact: true })).toBeVisible({ timeout: 15000 });
    ...
    await expect(page.getByText('Mis registros')).toBeVisible();
    ...
    await page.screenshot({ path: testInfo.outputPath('02-home-offline.png'), fullPage: true });
    ...
    await expect(page.getByText(/Guardado en este dispositivo|Pendiente de sincronización/)).toBeVisible({ timeout: 15000 });
    await page.screenshot({ path: testInfo.outputPath('03-observation-saved-local.png'), fullPage: true });
    await page.getByLabel('Cerrar relevamiento').click();
    await page.getByTitle('Más opciones').click();
    await page.getByRole('button', { name: /Mis registros/ }).click();
    await expect(page.getByText('Mis registros')).toBeVisible();
    await expect(page.getByText('Hongo de prueba')).toBeVisible();
    await page.screenshot({ path: testInfo.outputPath('04-my-records-offline.png'), fullPage: true });
    await page.getByTitle('Cerrar').click();
    ...
    await expect(page.getByTitle('Estado de tus registros')).toContainText('Sincronizado', { timeout: 30000 });
    await page.screenshot({ path: testInfo.outputPath('07-synchronized.png'), fullPage: true });
