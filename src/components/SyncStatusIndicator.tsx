...
export default function SyncStatusIndicator({ status }: { status: CanonicalSyncStatus }) {
  const Icon = status.state === 'ONLINE_SYNCED' ? Check : status.state === 'OFFLINE' ? CloudOff : status.state === 'ERROR' || status.state === 'BACKEND_UNAVAILABLE' ? TriangleAlert : LoaderCircle;
