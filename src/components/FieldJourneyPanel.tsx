...
import { createSyncIdentity, enqueueSyncDataset, getDeviceId, getLocalSyncEntity, makeSyncKey, syncQueued } from '../lib/remoteSync';
...
export default function FieldJourneyPanel({ user, onClose, onOpenSurvey, onOpenMap }: Props) {
  ...
  const canStart = true;
  const teamLabel = assignment?.expand?.team?.code || assignment?.expand?.event?.team_name || 'Equipo no asignado';
  const unitLabel = assignment?.expand?.site?.name || assignment?.expand?.site?.code || 'Unidad no asignada';
  const eventLabel = assignment?.expand?.event?.event_id || assignment?.event || 'Jornada sin código';
  ...
  const closeJourney = async () => {
    ...
    setMessage('Cierre pendiente de sincronización.');
  };

  const syncClosedJourney = async () => {
    setMessage('Sincronizando cierre...');
    try {
      const result = await syncQueued(pb);
      setMessage(result.errors || result.conflicts ? 'El cierre sigue pendiente de sincronización.' : 'Cierre sincronizado.');
    } catch { setMessage('El cierre sigue pendiente de sincronización.'); }
  };
  ...
  return <div className="fixed inset-0 z-[2500] overflow-y-auto bg-atlas-paper text-atlas-ink">
    <div className="mx-auto min-h-screen w-full max-w-2xl px-4 pb-10 sm:px-8">
      ...
      <div className={`mb-4 flex flex-col items-start gap-1 border px-3 py-3 font-sans text-xs sm:flex-row sm:items-center sm:justify-between ${syncStatus.state === 'OFFLINE' ? 'border-amber-700 bg-amber-50 text-amber-900' : 'border-atlas-ink/20'}`}>
        <span className="inline-flex items-center gap-2">{syncStatus.state === 'OFFLINE' ? <CloudOff className="h-4 w-4 shrink-0" /> : <RefreshCw className="h-4 w-4 shrink-0" />}{syncStatusLabel(syncStatus)}</span>
        <span className="shrink-0 font-mono">{syncStatus.pending_count} pendientes</span>
      </div>

      <section className="border-b border-atlas-ink/20 pb-5">
        <div className="flex items-start justify-between gap-4"><div><p className="font-mono text-[10px] uppercase tracking-wider opacity-55">Contexto de campo</p><p className="mt-1 font-sans text-sm font-bold">{user.displayName || user.email} · Relevador/a · {teamLabel}</p><h3 className="mt-3 font-serif text-xl italic">{assignment?.expand?.event?.title || 'Sin jornada asignada'}</h3><p className="mt-1 font-mono text-[10px] uppercase tracking-wider opacity-60">{eventLabel} · {unitLabel}</p></div><span className={`border px-2 py-1 font-mono text-[10px] uppercase ${journey.status === 'active' ? 'border-green-700 text-green-800' : journey.status === 'closed' ? 'border-atlas-ink/30 opacity-55' : 'border-atlas-earth text-atlas-earth'}`}>{journey.status === 'active' ? 'En curso' : journey.status === 'closed' ? 'Cerrada' : assignment ? 'Asignada' : 'Pendiente'}</span></div>
        <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3"><div><p className="font-mono text-[9px] uppercase opacity-45">Protocolo</p><p className="mt-1 font-sans text-sm">Asignado</p></div><div><p className="font-mono text-[9px] uppercase opacity-45">Inicio</p><p className="mt-1 font-sans text-sm">{journey.startedAt ? new Date(journey.startedAt).toLocaleTimeString('es-AR', { hour: '2-digit', minute: '2-digit' }) : 'Pendiente'}</p></div><div><p className="font-mono text-[9px] uppercase opacity-45">Estado</p><p className="mt-1 font-sans text-sm">{journey.status === 'active' ? 'Recorriendo' : journey.status === 'closed' ? 'Finalizada' : 'Lista para iniciar'}</p></div></div>
      </section>
      ...
      {journey.status === 'closed' && <section className="border-l-4 border-atlas-earth px-4 py-5"><h3 className="font-serif text-xl italic">Jornada cerrada</h3><p className="mt-1 font-sans text-sm opacity-70">{message || 'Esperando sincronización y revisión de coordinación.'}</p>{syncStatus.pending_count > 0 && syncStatus.state !== 'OFFLINE' && <button onClick={() => void syncClosedJourney()} className="mt-3 inline-flex items-center gap-2 border border-atlas-ink px-3 py-2 font-sans text-xs font-bold"><RefreshCw className="h-3 w-3" /> Sincronizar cierre</button>}</section>}
