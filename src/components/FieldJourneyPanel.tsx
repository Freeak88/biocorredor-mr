  ...
  return <div className="fixed inset-0 z-[2500] overflow-y-auto bg-atlas-paper text-atlas-ink">
    <div className="mx-auto min-h-screen w-full max-w-2xl px-4 pb-10 sm:px-8">
      ...
      <section className="border-b border-atlas-ink/20 pb-5">
        ...
        <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4"><div><p className="font-mono text-[9px] uppercase opacity-45">Sector</p><p className="mt-1 font-sans text-sm">{assignment?.expand?.site?.code || 'Pendiente'}</p></div><div><p className="font-mono text-[9px] uppercase opacity-45">Protocolo</p><p className="mt-1 font-sans text-sm">Asignado</p></div><div><p className="font-mono text-[9px] uppercase opacity-45">Equipo</p><p className="mt-1 font-sans text-sm">{assignment?.expand?.team?.code || assignment?.expand?.event?.team_name || 'Pendiente'}</p></div><div><p className="font-mono text-[9px] uppercase opacity-45">Inicio</p><p className="mt-1 font-sans text-sm">{journey.startedAt ? new Date(journey.startedAt).toLocaleTimeString('es-AR', { hour: '2-digit', minute: '2-digit' }) : 'Pendiente'}</p></div></div>
      </section>
